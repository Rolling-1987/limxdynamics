...
void RobotHWLoop::StartControlLoop(ros::NodeHandle& nh) {
  ...
  controllerManager_->loadController(std::string("/DampingHumanoidController"));
  ROS_INFO("Load controller : %s" , std::string("/DampingHumanoidController").c_str());
  controllerManager_->loadController(std::string("/StandHumanoidController"));
  ROS_INFO("Load controller : %s" , std::string("/StandHumanoidController").c_str());
  controllerManager_->loadController(std::string("/MimicHumanoidController"));
  ROS_INFO("Load controller : %s" , std::string("/MimicHumanoidController").c_str());

  // Load ros params
  int error = 0;

  loopHz_ = nh.param<double>("/robot_hw/loop_frequency", 1000);
  cycleTimeErrorThreshold_ = nh.param<double>("/robot_hw/cycle_time_error_threshold", 0.001);
